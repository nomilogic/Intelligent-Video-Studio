...
- **Template Library (~1,550).** `lib/templates.ts` keeps 4 hand-crafted templates plus 46 mid-sized procedurally-built entries (`buildTitleTpl()`, `buildSlideshowTpl()`, `buildLowerThirdTpl()`), plus a 1,500-entry bulk catalog via `buildBulkTemplates()` that cross-multiplies 25 themed `BULK_CATEGORIES` (Sale, Launch, Event, Quote, Tutorial, BTS, Recipe, Fitness, Listing, Wellness, Podcast, News, Travel, Wedding, Birthday, Music, Teaser, Coming Soon, Thank You, Subscribe, Tech, Beauty, Gaming, Holiday, Recap) × 12 title variants × 5 aspect ratios (9:16, 1:1, 16:9, 4:5, 21:9). The bulk constants must stay declared **above** the `export const TEMPLATES` line so they're past the const TDZ when `buildExtendedTemplates()` runs at module init. MediaPanel's Templates tab has a search box, aspect-ratio chip filter, and "Load more" pagination (60 per page) so the catalog stays browseable.
...
## Phase-5 Updates (April 2026) — Particles + 500 Transitions

### Particle effect layer (`mediaType: "particles"`)
- `lib/particles.ts` — 15 `ParticleDef` presets (snow, rain, confetti,
  stars, sparkles, bokeh, dust, fireflies, hearts, leaves, petals,
  embers, smoke, bubbles, ash). Shared deterministic PRNG
  (`hash01(seed)`) seeded from `clip.id` so Canvas preview and the MP4
  export render identically frame-for-frame.
- `resolveParticleClip(clip)` merges per-clip overrides on top of the
  preset defaults; `drawParticles(ctx, w, h, time, resolved)` draws one
  frame.
- New `Clip` fields: `particleKind`, `particleCount`, `particleSize`,
  `particleSpeed`, `particleColor`, `particleColor2`, `particleOpacity`,
  `particleSpread`, `particleDirection` (0–360), `particleGravity`,
  `particleTwinkle`.
- UI: emoji grid in `MediaPanel` (effects tab → Particles), and a
  `ParticleSection` in `PropertiesInspector` with kind picker, sliders
  and 2 color pickers.
- Render path: `Canvas.tsx` ParticlesOverlay + memo-skip; `use-export.ts`
  threads `time` into `drawClipToCanvas` and renders particles via the
  shared `drawParticles`.

### 500+ parametric transition presets
- `lib/transition-presets.ts` — 638 presets generated by 13 generators
  (Slide, Push, Wipe, Zoom, Blur, Spin, Iris, ColorFlash, DropPop,
  ShakeGlitch, SwirlMorph, Cinematic, Reveal). Exports
  `TRANSITION_PRESETS`, `TRANSITION_PRESET_CATEGORIES`,
  `getTransitionPreset(key)`.
- New transition `type: "param"` with `ParamTransitionParams` carrying
  `slideX/slideY/scale/rotate/blur/fade/shake/wipe/iris/colorOverlay`.
  `ClipTransition` now optionally carries `params` and `presetKey`.
- `animation.ts` `getTransitionMod` has a full `param` branch composing
  all of the above into a single `TransitionMod`. `getActiveTransition`
  forwards `tr.params` so both Canvas + export use the same composer.
- UI: `TransitionPresetPicker` in `PropertiesInspector` (search +
  category chips + capped grid + Clear).
- `lib/ai-schema.ts` exposes `transitionPresets`,
