...
import {
  ...
  MASK_GROUP_ORDER,
  type MaskPreset,
} from "../lib/mask-library";
import { textContainerStyle, textElementStyle } from "../lib/text-style";
import {
  EFFECT_LIBRARY as EFFECT_CATALOG,
  EFFECT_CATEGORIES,
} from "../lib/effect-library";
import {
  TRANSITION_LIBRARY as TRANSITION_CATALOG,
...
const TRANSITIONS = TRANSITION_CATALOG.map((t) => ({
  value: t.type,
  label: t.label,
  category: t.category,
}));
...
function Section({
  ...
  const key =
    storageKey ?? `pi-section-${title.replace(/\s+/g, "-").toLowerCase()}`;
      ...
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter") {
            ...
            const next = Math.max(
              min,
              Math.min(max, value + dir * step * mult),
            );
            setDraft(null);
            onChange(next);
          }
        ...
        <Label className="text-muted-foreground flex items-center gap-1">
          ...
          {onToggleTween && (
            <button
              ...
              title={
                tweened
                  ? "Tween ON — animates between keyframes. Click to turn off."
                  : "Tween OFF — values hold/snap between keyframes. Click to animate."
              }
...
function CanvasFitAlignSection({
  ...
  const aspect =
    clip.width > 0 && clip.height > 0 ? clip.width / clip.height : 1;
  ...
  return (
    <Section title="Canvas Fit & Align">
      ...
      <p className="text-[9px] text-muted-foreground leading-snug">
        Fill stretches edge-to-edge. Fit/Cover preserve aspect (Fit shows the
        whole clip, Cover crops to fill). Snap moves the clip while keeping its
        current size.
      </p>
    </Section>
  );
...
function MaskSection({
  ...
  const setMask = (patch: Partial<ClipMask> | null) => {
    if (patch === null) {
      ...
      dispatch({
        type: "UPDATE_CLIP",
        payload: { id: clip.id, updates: { mask: null as any } },
      });
      return;
    }
    ...
    dispatch({
      type: "UPDATE_CLIP",
      payload: { id: clip.id, updates: { mask: next } },
    });
  };
    ...
    >
      {!m ? (
        <p className="text-[10px] text-muted-foreground">
          Pick a preset or upload an image. Black is hidden, white is visible —
          grey gradients give partial transparency.
        </p>
      ) : null}
        ...
        <>
          <div className="grid grid-cols-2 gap-1.5">
            <div>
              ...
              <Select
                value={m.mode}
                onValueChange={(v: any) => setMask({ mode: v })}
              >
          ...
          */}
          {clip.mediaType === "maskLayer" && (
            <div className="space-y-1 pt-1 border-t border-border/40">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    dispatch({
                      ...
                      payload: {
                        id: clip.id,
                        updates: { maskAffectsTracksBelow: n },
                      },
                    });
                  }}
              ...
              <p className="text-[9px] text-muted-foreground leading-snug">
                Set to 0 to mask everything underneath. Set to a number to limit
                the cutout to just the N tracks directly below this mask (e.g. 1
                only cuts the track right beneath it).
              </p>
            </div>
          )}
        </>
...
function EffectsSection({
  ...
  const writeEffects = (next: Effect[]) => {
    dispatch({
      type: "UPDATE_CLIP",
      payload: { id: clip.id, updates: { effects: next } },
    });
  };
  ...
  const removeEffect = (id: string) =>
    writeEffects(effects.filter((e) => e.id !== id));
  ...
  return (
    ...
    >
                  ...
                  return (
                    <Button
                      ...
                      onClick={() =>
                        active
                          ? removeEffect(
                              effects.find((ef) => ef.type === e.type)!.id,
                            )
                          : addEffect(e.type)
                      }
      ...
      {effects.length === 0 && (
        <p className="text-[10px] text-muted-foreground">
          Tap an effect to add it. Stack as many as you like.
        </p>
      )}
      {effects.map((eff) => {
        ...
        return (
          <div
            key={eff.id}
            className="rounded-md border border-white/10 bg-black/20 p-1.5 space-y-1.5"
          >
            ...
            {supportsColor && (
              <div className="flex items-center gap-1.5">
                <Label className="text-[10px] text-muted-foreground w-14">
                  Color
                </Label>
                <Input
                  type="color"
                  ...
                  onChange={(e) =>
                    updateEffect(eff.id, { color: e.target.value })
                  }
          ...
          >
            <svg
              ...
              dangerouslySetInnerHTML={{
                __html: `<g fill="currentColor">${s.svg}</g>`,
              }}
            />
          </button>
        ))}
...
function ParticleSection({
  ...
  return (
    <Section title="Particles">
      ...
      <NumPair
        label="Count"
        value={clip.particleCount ?? def.defaults.count}
        min={5}
        max={400}
        step={1}
      ...
      <NumPair
        label="Size"
        value={clip.particleSize ?? def.defaults.size}
        min={1}
        max={120}
        step={1}
      ...
      <NumPair
        label="Speed"
        value={clip.particleSpeed ?? def.defaults.speed}
        min={0.1}
        max={3}
        step={0.05}
      ...
      <NumPair
        label="Spread"
        value={clip.particleSpread ?? def.defaults.spread}
        min={0}
        max={1}
        step={0.05}
        onChange={(v) => update({ particleSpread: v })}
      />
      <NumPair
        label="Gravity"
        value={clip.particleGravity ?? def.defaults.gravity}
        min={-2}
        max={2}
        step={0.1}
        onChange={(v) => update({ particleGravity: v })}
      />
      <NumPair
        label="Twinkle"
        value={clip.particleTwinkle ?? def.defaults.twinkle}
        min={0}
        max={1}
        step={0.05}
      ...
      <NumPair
        label="Opacity"
        value={clip.particleOpacity ?? def.defaults.opacity}
        min={0}
        max={1}
        step={0.05}
      ...
      <div className="flex items-center gap-2">
        ...
        <Select
          ...
          onValueChange={(v) =>
            update({
              particleDirection: v as NonNullable<Clip["particleDirection"]>,
            })
          }
        >
      ...
      <div className="flex items-center gap-2">
        ...
        <Input
          type="color"
          value={
            clip.particleColor2 ?? def.defaults.color2 ?? def.defaults.color
          }
...
function WavesSection({
  ...
  return (
    <Section title="Waves">
      ...
      <NumPair
        label="Count"
        value={clip.waveCount ?? def?.defaults.count ?? 3}
        min={1}
        max={8}
        step={1}
        onChange={(v) => update({ waveCount: v })}
      />
      <NumPair
        label="Amplitude"
        value={clip.waveAmplitude ?? def?.defaults.amplitude ?? 0.15}
        min={0}
        max={1}
        step={0.01}
      ...
      <NumPair
        label="Frequency"
        value={clip.waveFrequency ?? def?.defaults.frequency ?? 2}
        min={0.5}
        max={10}
        step={0.1}
      ...
      <NumPair
        label="Speed"
        value={clip.waveSpeed ?? def?.defaults.speed ?? 1}
        min={0.1}
        max={5}
        step={0.05}
      ...
      <NumPair
        label="Opacity"
        value={clip.waveOpacity ?? def?.defaults.opacity ?? 0.9}
        min={0}
        max={1}
        step={0.05}
        onChange={(v) => update({ waveOpacity: v })}
      />
      <div className="flex items-center gap-2">
        <Label className="text-[10px] text-muted-foreground w-14">
          Direction
        ...
        <Select
          value={clip.waveDirection ?? def?.defaults.direction ?? "horizontal"}
          onValueChange={(v) =>
            update({ waveDirection: v as "horizontal" | "vertical" | "radial" })
          }
        >
          <SelectTrigger className="h-7 text-xs flex-1">
            <SelectValue />
      ...
      <div className="flex items-center gap-2">
        ...
        <input
          type="checkbox"
          checked={clip.waveFill ?? def?.defaults.fill ?? true}
          onChange={(e) => update({ waveFill: e.target.checked })}
        ...
        <input
          type="color"
          value={clip.waveColor ?? def?.defaults.color ?? "#0ea5e9"}
          onChange={(e) =>
            update({ waveColor: e.target.value, color: e.target.value })
          }
          className="h-7 w-10 rounded border border-border p-0.5 cursor-pointer"
        />
        <Label className="text-[10px] text-muted-foreground w-10">
          Color 2
        </Label>
        <input
          type="color"
          value={clip.waveColor2 ?? def?.defaults.color2 ?? "#0284c7"}
          onChange={(e) => update({ waveColor2: e.target.value })}
          className="h-7 w-10 rounded border border-border p-0.5 cursor-pointer"
        />
      </div>
    </Section>
...
function GradientSection({
  ...
  return (
    <Section title="Gradient">
      <div
        className="w-full h-14 rounded-lg border border-border mb-1"
        style={{ background: bg }}
      />
      <div className="flex items-center gap-2">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="conic" className="text-xs">
              Conic
            </SelectItem>
          </SelectContent>
        </Select>
        ...
        {stops.map(([pos, color], i) => (
          <div key={i} className="flex items-center gap-2">
            <input
              ...
              onChange={(e) => {
                const s = [...stops];
                s[i] = [s[i][0], e.target.value];
                update({ gradientStops: s as [number, string][] });
              }}
              className="h-7 w-10 rounded border border-border p-0.5 cursor-pointer"
            />
      ...
      <p className="text-[10px] text-muted-foreground">
        Open the Gradient Editor from Effects tab for full stop control.
      </p>
    </Section>
  );
}
  ...
  return (
    <Section title="Visualizer">
      <div className="flex items-center gap-2">
        ...
        <Select
          value={clip.visualizerKind ?? "bars"}
          onValueChange={(v) =>
            update({
              visualizerKind: v as
                | "bars"
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="bars" className="text-xs">
              Bars
            </SelectItem>
            ...
            <SelectItem value="spectrum" className="text-xs">
              Spectrum
            </SelectItem>
      ...
      <div className="flex items-center gap-2">
        ...
        <input
          type="color"
          value={clip.visualizerColor ?? "#22d3ee"}
          onChange={(e) =>
            update({ visualizerColor: e.target.value, color: e.target.value })
          }
          className="h-7 w-10 rounded border border-border p-0.5 cursor-pointer"
        />
        <Label className="text-[10px] text-muted-foreground w-14">
        ...
        <input
          type="color"
          value={clip.visualizerColor2 ?? "#818cf8"}
            ...
            onClick={() => {
              ...
              update({
                transitionIn: {
                  type: "param",
                  duration:
                    preset.duration ?? clip.transitionIn?.duration ?? 0.6,
...
function SavedPresetsSection({
  ...
  const onApply = (p: CustomPreset) => {
    dispatch({
      type: "UPDATE_CLIP",
      payload: { id: clip.id, updates: p.data },
    });
  };
  ...
  return (
    <div className="space-y-1.5">
      ...
      {presets.length === 0 ? (
        <p className="text-[10px] text-muted-foreground">
          No presets saved. Save the current clip's styling to reuse later.
        </p>
      ) : (
        <div className="space-y-1 max-h-40 overflow-y-auto pr-1">
          {presets.map((p) => (
            <div
              key={p.id}
              className="flex items-center gap-1 rounded border border-border bg-muted/20 px-1.5 py-1"
            >
              <button
                onClick={() => onApply(p)}
...
function ChromaKeySection({
  ...
  return (
    ...
    >
      ...
      />
      <p className="text-[10px] text-muted-foreground leading-snug">
        Pick the background color (or a swatch). Threshold sets which pixels
        become transparent; smoothness softens the edges; spill suppresses
        leftover color cast on the subject.
      </p>
    </Section>
  );
}
    ...
    <Section title="Blur Region">
      <NumPair
        ...
        onChange={(v) =>
          dispatch({
            type: "UPDATE_CLIP",
            payload: { id: clip.id, updates: { blurAmount: v } },
          })
        }
      />
      <NumPair
        ...
        onChange={(v) =>
          dispatch({
            type: "UPDATE_CLIP",
            payload: { id: clip.id, updates: { borderRadius: v } },
          })
        }
      />
      <p className="text-[10px] text-muted-foreground leading-snug">
        Drops a blurred rectangle on top of the composite — handy for hiding
        logos, faces or text. Animate position and size with keyframes (Basic
        tab) to track moving objects.
      </p>
    </Section>
  );
}
  ...
  return (
    <Section title="Split">
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-1.5">
          <Input
            ...
            value={parts}
            onChange={(e) =>
              setParts(Math.max(2, Math.min(64, parseInt(e.target.value) || 2)))
            }
        ...
        <div className="flex items-center gap-1.5">
          <Input
            ...
            value={every}
            onChange={(e) =>
              setEvery(Math.max(0.2, parseFloat(e.target.value) || 1))
            }
            className="h-7 text-xs w-14"
          />
          <Button
            ...
            onClick={() =>
              dispatch({
                type: "SPLIT_EVERY",
                payload: { clipId: clip.id, seconds: every },
              })
            }
      ...
      <p className="text-[10px] text-muted-foreground leading-relaxed pt-1">
        Tip: Use the AI bar to "split clip at scenes", "split into 5
        highlights", or "cut every 2 seconds for a montage".
      </p>
    </Section>
  );
}
...
export default function PropertiesInspector({
  ...
  const resolved = clip
    ? resolveClip(clip, state.keyframes, state.currentTime)
    : null;
  ...
  const addKeyframe = (property: string) => {
    ...
    const existing = state.keyframes.find(
      (k) => k.clipId === clip.id && k.property === property,
    );
  ...
  const isTweened = (property: string) =>
    clip
      ? state.keyframes.some(
          (k) =>
            k.clipId === clip.id &&
  ...
  const toggleTween = (property: string) => {
    if (!clip) return;
    const kfs = state.keyframes.filter(
      (k) => k.clipId === clip.id && k.property === property,
    );
    ...
    for (const kf of kfs) {
      dispatch({
        ...
        payload: { id: kf.id, easing: nextEasing },
      });
    }
  };
  ...
  const isAtKfTime = clip
    ? state.keyframes.some(
        (k) =>
          k.clipId === clip.id && Math.abs(k.time - state.currentTime) < 0.02,
      )
    : false;
  ...
  return (
    <div
      ...
      className="w-[100%] h-[100%] flex flex-col border-l border-border bg-card shrink-0 overflow-hidden"
    >
        ...
        {clip && (
          <div className="flex gap-0.5">
            ...
            <Button
              ...
              size="icon"
              className="w-6 h-6"
              onClick={() => update({ hidden: !clip.hidden })}
              title={clip.hidden ? "Show" : "Hide"}
            >
              {clip.hidden ? (
            ...
            <Button
              ...
              size="icon"
              className="w-6 h-6"
              onClick={() => update({ locked: !clip.locked })}
              ...
              <div className="flex items-center gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    dispatch({
                      type: "SET_DURATION",
                      payload: parseFloat(e.target.value) || 30,
                    })
                  }
                  className="h-7 text-xs"
                />
                ...
                <div className="grid grid-cols-4 gap-1 mt-1">
                  {[
                    ...
                    <button
                      key={g}
                      onClick={() =>
                        dispatch({ type: "SET_BACKGROUND", payload: g })
                      }
          ...
          <Section title="Aspect Ratio Presets">
            <div className="grid grid-cols-3 gap-1.5">
              {[
                ...
                const active =
                  state.canvasWidth === r.w && state.canvasHeight === r.h;
                const ratio = r.w / r.h;
                const maxDim = 22;
          ...
          <Section title="Markers">
            <div className="space-y-1 max-h-40 overflow-y-auto">
              {(state.markers || []).length === 0 && (
                <p className="text-[10px] text-muted-foreground">
                  No markers. Press M or Shift+click ruler.
                </p>
              )}
              {(state.markers || [])
                ...
                .map((m) => (
                  <div
                    key={m.id}
                    className="flex items-center gap-2 text-[10px] bg-muted/30 px-2 py-1 rounded group"
                  >
                    <div
          ...
          {isAtKfTime && (
            <div className="mx-2 mt-2 px-2 py-1 rounded bg-yellow-400/15 border border-yellow-400/40 flex items-center gap-1.5">
              ...
              <p className="text-[10px] text-yellow-300 leading-tight">
                <span className="font-semibold">Keyframe mode</span> — slider
                changes update the keyframe at {state.currentTime.toFixed(2)}s
              </p>
            </div>
          )}
          <TabsList className="grid grid-cols-4 mx-2 mt-2 h-8">
            <TabsTrigger value="basic" className="text-[10px]">
              Basic
            </TabsTrigger>
            ...
            <TabsContent value="basic" className="m-0">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        update({ startTime: parseFloat(e.target.value) })
                      }
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        update({ duration: parseFloat(e.target.value) })
                      }
                      className="h-7 text-xs"
                    />
                  </div>
                ...
                {(clip.mediaType === "video" || clip.mediaType === "image") &&
                  clip.src && (
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-muted-foreground/70 truncate font-mono leading-tight">
              ...
              */}
                ...
                <>
                  ...
                  <Section title="Adjustments">
                    <p className="text-[10px] text-muted-foreground leading-relaxed">
                      Non-destructive color grading applied to all clips beneath
                      this layer.
                    </p>
                    {(
                      [
                        ...
                        key: keyof import("../lib/types").AdjustmentSettings;
                        label: string;
                        min: number;
                        max: number;
                        def: number;
              ...
              <Section
                ...
                action={
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() =>
                        update({ preserveRatio: !clip.preserveRatio })
                      }
                      title={
                        clip.preserveRatio
                          ? "Aspect ratio is locked — drag handles preserve W:H. Click to unlock."
                          : "Lock aspect ratio so resize keeps W:H. Hold Shift while dragging for one-off lock."
                      }
              ...
              >
                <NumPair
                  label="X"
                  value={liveVal("x", clip.x)}
                  min={-1}
                  max={2}
                  step={0.01}
                  onChange={(v) => updateAnimatable("x", v)}
                  onKeyframe={() => addKeyframe("x")}
                  hasKeyframe={hasKf("x")}
                  isAtKeyframe={isAtKf("x")}
                  tweened={isTweened("x")}
                  onToggleTween={() => toggleTween("x")}
                />
                <NumPair
                  label="Y"
                  value={liveVal("y", clip.y)}
                  min={-1}
                  max={2}
                  step={0.01}
                  onChange={(v) => updateAnimatable("y", v)}
                  onKeyframe={() => addKeyframe("y")}
                  hasKeyframe={hasKf("y")}
                  isAtKeyframe={isAtKf("y")}
                  tweened={isTweened("y")}
                  onToggleTween={() => toggleTween("y")}
                />
                <NumPair
                  label="W"
                  value={liveVal("width", clip.width)}
                  min={0.01}
                  max={2}
                  step={0.01}
                  onChange={(v) => {
                  ...
                  onKeyframe={() => addKeyframe("width")}
                  hasKeyframe={hasKf("width")}
                  isAtKeyframe={isAtKf("width")}
                  tweened={isTweened("width")}
                  onToggleTween={() => toggleTween("width")}
                />
                <NumPair
                  label="H"
                  value={liveVal("height", clip.height)}
                  min={0.01}
                  max={2}
                  step={0.01}
                  onChange={(v) => {
                  ...
                  onKeyframe={() => addKeyframe("height")}
                  hasKeyframe={hasKf("height")}
                  isAtKeyframe={isAtKf("height")}
                  tweened={isTweened("height")}
                  onToggleTween={() => toggleTween("height")}
                />

                <NumPair
                  label="Rotation"
                  value={liveVal("rotation", clip.rotation)}
                  min={-180}
                  max={180}
                  step={1}
                  suffix="°"
                  onChange={(v) => updateAnimatable("rotation", v)}
                  onKeyframe={() => addKeyframe("rotation")}
                  hasKeyframe={hasKf("rotation")}
                  isAtKeyframe={isAtKf("rotation")}
                  tweened={isTweened("rotation")}
                  onToggleTween={() => toggleTween("rotation")}
                />
                <NumPair
                  label="Scale"
                  value={liveVal("scale", clip.scale)}
                  min={0.1}
                  max={3}
                  step={0.05}
                  suffix="x"
                  onChange={(v) => updateAnimatable("scale", v)}
                  onKeyframe={() => addKeyframe("scale")}
                  hasKeyframe={hasKf("scale")}
                  isAtKeyframe={isAtKf("scale")}
                  tweened={isTweened("scale")}
                  onToggleTween={() => toggleTween("scale")}
                  snapPoints={[1]}
                />
                <NumPair
                  label="Opacity"
                  value={liveVal("opacity", clip.opacity)}
                  min={0}
                  max={1}
                  step={0.01}
                  onChange={(v) => updateAnimatable("opacity", v)}
                  onKeyframe={() => addKeyframe("opacity")}
                  hasKeyframe={hasKf("opacity")}
                  isAtKeyframe={isAtKf("opacity")}
                  tweened={isTweened("opacity")}
                  onToggleTween={() => toggleTween("opacity")}
                  snapPoints={[1]}
                />

                <div className="flex gap-1.5">
                  <Button
                    variant={clip.flipH ? "secondary" : "outline"}
                    size="sm"
                    className="h-7 text-xs flex-1"
                    onClick={() => update({ flipH: !clip.flipH })}
                  >
                    <FlipHorizontal2 className="w-3 h-3 mr-1" /> Flip H
                  </Button>
                  <Button
                    variant={clip.flipV ? "secondary" : "outline"}
                    size="sm"
                    className="h-7 text-xs flex-1"
                    onClick={() => update({ flipV: !clip.flipV })}
                  >
              ...
              {clip.mediaType === "maskLayer" && (
                <>
                  ...
                  <div className="px-3 py-3 text-[10px] text-muted-foreground leading-snug">
                    This mask layer cuts out the visible composite of all media
                    beneath it. Use the Basic tab to keyframe its position,
                    size, rotation and scale; the cutout shape will animate
                    along with the clip rectangle.
                  </div>
                </>
              )}
              {clip.mediaType === "logoBlur" && (
                <>
                  ...
                  <Separator />
                  <div className="px-3 py-3 text-[10px] text-muted-foreground leading-snug">
                    Use the Basic tab to keyframe X / Y / W / H to track moving
                    logos and faces.
                  </div>
                </>
              )}
                ...
                clip.mediaType !== "logoBlur" && (
                  <>
                            ...
                            onClick={() =>
                              dispatch({
                                ...
                                payload: [
                                  {
                                    type: "applyPreset",
                                    payload: { clipId: clip.id, preset: p.key },
                                  },
                                ],
                              })
                            }
                    ...
                    >
                      <NumPair
                        label="Brightness"
                        value={liveVal("brightness", clip.filters.brightness)}
                        min={0}
                        max={200}
                        step={1}
                        suffix="%"
                        onChange={(v) => updateFilter("brightness", v)}
                        onKeyframe={() => addKeyframe("brightness")}
                        hasKeyframe={hasKf("brightness")}
                        isAtKeyframe={isAtKf("brightness")}
                        tweened={isTweened("brightness")}
                        onToggleTween={() => toggleTween("brightness")}
                        snapPoints={[100]}
                      />
                      <NumPair
                        label="Contrast"
                        value={liveVal("contrast", clip.filters.contrast)}
                        min={0}
                        max={200}
                        step={1}
                        suffix="%"
                        onChange={(v) => updateFilter("contrast", v)}
                        onKeyframe={() => addKeyframe("contrast")}
                        hasKeyframe={hasKf("contrast")}
                        isAtKeyframe={isAtKf("contrast")}
                        tweened={isTweened("contrast")}
                        onToggleTween={() => toggleTween("contrast")}
                        snapPoints={[100]}
                      />
                      <NumPair
                        label="Saturation"
                        value={liveVal("saturation", clip.filters.saturation)}
                        min={0}
                        max={200}
                        step={1}
                        suffix="%"
                        onChange={(v) => updateFilter("saturation", v)}
                        onKeyframe={() => addKeyframe("saturation")}
                        hasKeyframe={hasKf("saturation")}
                        isAtKeyframe={isAtKf("saturation")}
                        tweened={isTweened("saturation")}
                        onToggleTween={() => toggleTween("saturation")}
                        snapPoints={[100]}
                      />
                      <NumPair
                        label="Hue"
                        value={liveVal("hue", clip.filters.hue)}
                        min={-180}
                        max={180}
                        step={1}
                        suffix="°"
                        onChange={(v) => updateFilter("hue", v)}
                        onKeyframe={() => addKeyframe("hue")}
                        hasKeyframe={hasKf("hue")}
                        isAtKeyframe={isAtKf("hue")}
                        tweened={isTweened("hue")}
                        onToggleTween={() => toggleTween("hue")}
                      />
                      <NumPair
                        label="Blur"
                        value={liveVal("blur", clip.filters.blur)}
                        min={0}
                        max={20}
                        step={0.5}
                        suffix="px"
                        onChange={(v) => updateFilter("blur", v)}
                        onKeyframe={() => addKeyframe("blur")}
                        hasKeyframe={hasKf("blur")}
                        isAtKeyframe={isAtKf("blur")}
                        tweened={isTweened("blur")}
                        onToggleTween={() => toggleTween("blur")}
                      />
                      <NumPair
                        label="Grayscale"
                        value={liveVal("grayscale", clip.filters.grayscale)}
                        min={0}
                        max={100}
                        step={1}
                        suffix="%"
                        onChange={(v) => updateFilter("grayscale", v)}
                        onKeyframe={() => addKeyframe("grayscale")}
                        hasKeyframe={hasKf("grayscale")}
                        isAtKeyframe={isAtKf("grayscale")}
                        tweened={isTweened("grayscale")}
                        onToggleTween={() => toggleTween("grayscale")}
                      />
                      <NumPair
                        label="Sepia"
                        value={liveVal("sepia", clip.filters.sepia)}
                        min={0}
                        max={100}
                        step={1}
                        suffix="%"
                        onChange={(v) => updateFilter("sepia", v)}
                        onKeyframe={() => addKeyframe("sepia")}
                        hasKeyframe={hasKf("sepia")}
                        isAtKeyframe={isAtKf("sepia")}
                        tweened={isTweened("sepia")}
                        onToggleTween={() => toggleTween("sepia")}
                      />
                      <NumPair
                        label="Invert"
                        value={liveVal("invert", clip.filters.invert)}
                        min={0}
                        max={100}
                        step={1}
                        suffix="%"
                        onChange={(v) => updateFilter("invert", v)}
                        onKeyframe={() => addKeyframe("invert")}
                        hasKeyframe={hasKf("invert")}
                        isAtKeyframe={isAtKf("invert")}
                        tweened={isTweened("invert")}
                        onToggleTween={() => toggleTween("invert")}
                      />
                    </Section>
                    ...
                    <Section title="Blend">
                      <Select
                        ...
                        onValueChange={(v) => update({ blendMode: v })}
                      >
                        <SelectTrigger
                          className="h-7 text-xs"
                          data-testid="select-blend-mode"
                        >
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                        <Crop className="w-3 h-3" /> Zoom into a region (0–1
                        fractions of source).
                      </p>
                      <NumPair
                        label="Crop X"
                        ...
                        step={0.01}
                        onChange={(v) =>
                          update({ cropX: Math.min(v, 1 - clip.cropWidth) })
                        }
                      />
                      <NumPair
                        label="Crop Y"
                        ...
                        step={0.01}
                        onChange={(v) =>
                          update({ cropY: Math.min(v, 1 - clip.cropHeight) })
                        }
                      />
                      <NumPair
                        label="Crop W"
                        ...
                        step={0.01}
                        onChange={(v) =>
                          update({ cropWidth: Math.min(v, 1 - clip.cropX) })
                        }
                      />
                      <NumPair
                        label="Crop H"
                        ...
                        step={0.01}
                        onChange={(v) =>
                          update({ cropHeight: Math.min(v, 1 - clip.cropY) })
                        }
                      />
                      <div className="grid grid-cols-3 gap-1">
                        <Button
            ...
            <TabsContent value="anim" className="m-0">
              <Section title="Transition In">
                ...
                >
                  <SelectTrigger
                    className="h-7 text-xs"
                    data-testid="select-transition-in"
                  >
                    <SelectValue />
                  </SelectTrigger>
                  ...
                  onChange={(v) =>
                    update({
                      transitionIn: {
                        ...
                        presetKey: clip.transitionIn?.presetKey,
                      },
                    })
                  }
                />
                <p className="text-[10px] text-muted-foreground">
                  Blends with the previous clip on the same track during the
                  first {(clip.transitionIn?.duration ?? 0.5).toFixed(1)}s.
                </p>
              </Section>
              ...
              */}
              ...
              <Section title="Animation In">
                <Select
                  ...
                  onValueChange={(v) => update({ animationIn: v })}
                >
                  <SelectTrigger
                    className="h-7 text-xs"
                    data-testid="select-animation-in"
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <NumPair
                  label="Duration"
                  value={clip.animationInDuration}
                  min={0.1}
                  max={5}
                  step={0.1}
                  suffix="s"
                  onChange={(v) => update({ animationInDuration: v })}
                />
              </Section>
              ...
              <Section title="Animation Out">
                <Select
                  ...
                  onValueChange={(v) => update({ animationOut: v })}
                >
                  <SelectTrigger
                    className="h-7 text-xs"
                    data-testid="select-animation-out"
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <NumPair
                  label="Duration"
                  value={clip.animationOutDuration}
                  min={0.1}
                  max={5}
                  step={0.1}
                  suffix="s"
                  onChange={(v) => update({ animationOutDuration: v })}
                />
              </Section>
              ...
              {state.keyframes.filter((k) => k.clipId === clip.id).length >
                0 && (
                <>
                  <Separator />
                  <Section title="Keyframes">
                    <div className="space-y-1 max-h-52 overflow-y-auto">
                      {state.keyframes
                        ...
                        .map((kf) => (
                          <div
                            key={kf.id}
                            className="bg-muted/30 rounded overflow-hidden group"
                          >
                            <div className="flex items-center gap-1.5 px-2 py-1 text-[10px]">
                              ...
                              <span className="text-yellow-300 font-medium min-w-[56px]">
                                {kf.property}
                              </span>
                              <button
                                ...
                                <SelectContent className="text-xs max-h-52">
                                  {(
                                    ...
                                    value === null ? (
                                      <div
                                        key={label}
                                        className="px-2 py-0.5 text-[9px] text-muted-foreground font-medium tracking-wider"
                                      >
                                        {label}
                                      </div>
                                    ) : (
                    ...
                    <p className="text-[9px] text-muted-foreground mt-1 leading-relaxed">
                      Drag diamonds on timeline to reposition · right-click to
                      delete
                    </p>
                  </Section>
                </>
            ...
            <TabsContent value="audio" className="m-0">
              <Section title="Volume">
                <NumPair
                  ...
                  onKeyframe={() => addKeyframe("volume")}
                  hasKeyframe={hasKf("volume")}
                  isAtKeyframe={isAtKf("volume")}
                  snapPoints={[1]}
                />
                <Button
...
const TEXT_PRESETS: {
  ...
  },
  {
    ...
    style: {
      ...
      color: "#ff0099",
      align: "center",
      glow: { enabled: true, color: "#ff0099", blur: 20, intensity: 3 },
    },
  },
  {
    key: "lower-third",
  ...
  },
  {
    ...
    style: {
      ...
      color: "#ffffff",
      align: "center",
      stroke: { enabled: true, color: "#ff0000", width: 5 },
    },
  },
  {
    key: "social",
    label: "Social Media Pop",
    style: {
      fontFamily: "Poppins, sans-serif",
      fontSize: 90,
      fontWeight: 800,
  ...
  },
  {
    ...
    style: {
      ...
      color: "#00ff88",
      align: "center",
      glow: { enabled: true, color: "#00ff88", blur: 12, intensity: 2 },
    },
  },
  {
    key: "bold-red",
    label: "Bold Red Alert",
    style: {
...
function MotionPathSection({
  ...
  const apply = () => {
    ...
    buildMotionKeyframes(selectedPath, clip).forEach(
  ...
  return (
    <Section title="Motion Path">
      ...
      <p className="text-[10px] text-muted-foreground leading-snug">
        Adds x/y/scale/rotation keyframes. Undo to revert. Fine-tune in the
        keyframe list below.
      </p>
      <Button
        variant="default"
  ...
  return (
    <>
      <Section title="Style Preset">
        <div className="grid grid-cols-2 gap-1.5 max-h-64 overflow-y-auto pr-0.5">
          {TEXT_PRESETS.map((p) => {
            ...
            const previewPx = Math.min(
              Math.max((merged.fontSize || 40) / 5.5, 11),
              17,
            );
            ...
            return (
              ...
              >
                ...
                <p className="text-[9px] text-muted-foreground px-1.5 pb-1 truncate">
                  {p.label}
                </p>
              </button>
            );
        ...
        <p className="text-[10px] text-muted-foreground">
          Click a preset to apply font, size, color &amp; effects — then
          customize freely.
        </p>
      </Section>
      ...
      <Section title="Content">
        ...
        <div className="flex items-center gap-2 rounded border border-white/10 bg-black/20 px-2 py-1.5">
          ...
          />
          <label
            htmlFor="text-auto-scale"
            className="text-[10px] flex-1 cursor-pointer"
          >
      ...
      <Section title="Font">
        <Select
          value={ts.fontFamily}
          onValueChange={(v) => setTs({ fontFamily: v })}
        >
          <SelectTrigger className="h-7 text-xs">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="max-h-72">
            {(["Sans", "Serif", "Display", "Handwriting", "Mono"] as const).map(
              (cat) => (
                <div key={cat}>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setTs({ fontSize: parseInt(e.target.value) || 64 })
              }
              className="h-7 text-xs"
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <div>
            ...
            <Input
              ...
              step={0.5}
              onChange={(e) =>
                setTs({ letterSpacing: parseFloat(e.target.value) || 0 })
              }
              className="h-7 text-xs"
            />
          </div>
          <div>
            ...
            <Input
              ...
              step={0.05}
              onChange={(e) =>
                setTs({ lineHeight: parseFloat(e.target.value) || 1.1 })
              }
              className="h-7 text-xs"
            />
          </div>
        ...
        <Select
          ...
          onValueChange={(v: any) => setTs({ align: v })}
        >
          <SelectTrigger className="h-7 text-xs">
            <SelectValue />
          </SelectTrigger>
      ...
      <Section title="Fill">
        ...
        <ToggleHeader
          ...
          onToggle={(v) =>
            setTs({
              gradient: v
                ? { ...DEFAULT_GRADIENT, ...grad, enabled: true }
                : { ...grad, enabled: false },
            })
          }
        />
      ...
      <Section title="Stroke">
        <ToggleHeader
          ...
          onToggle={(v) =>
            setTs({
              stroke: v
                ? { ...DEFAULT_STROKE, ...stroke, enabled: true }
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                value={stroke.color}
                onChange={(e) =>
                  setTs({ stroke: { ...stroke, color: e.target.value } })
                }
                className="h-7 w-10 rounded border border-border"
              />
              <Input
      ...
      <Section title="Glow">
        <ToggleHeader
          ...
          onToggle={(v) =>
            setTs({
              glow: v
                ? { ...DEFAULT_GLOW, ...glow, enabled: true }
                : { ...glow, enabled: false },
            })
          }
        />
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                value={glow.color}
                onChange={(e) =>
                  setTs({ glow: { ...glow, color: e.target.value } })
                }
                className="h-7 w-10 rounded border border-border"
              />
              <Input
      ...
      <Section title="Drop Shadow">
        <ToggleHeader
          ...
          onToggle={(v) =>
            setTs({
              textShadow: v
                ? { ...DEFAULT_SHADOW, ...shadow, enabled: true }
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <input
                type="color"
                value={
                  shadow.color.length > 7
                    ? shadow.color.slice(0, 7)
                    : shadow.color
                }
                onChange={(e) =>
                  setTs({ textShadow: { ...shadow, color: e.target.value } })
                }
                className="h-7 w-10 rounded border border-border"
              />
              <Input
      ...
      <Section title="Curve">
        ...
        />
        <p className="text-[10px] text-muted-foreground leading-snug">
          0° = straight. Positive bends down (smile), negative bends up (frown).
          At ±180° text wraps half a circle.
        </p>
      </Section>
          ...
          onToggle={(v) =>
            setTs({
              bg: {
                ...bg,
                gradient: v
                  ? { ...DEFAULT_BG_GRADIENT, ...bg.gradient, enabled: true }
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <div>
            ...
            <Input
              ...
              value={bg.padding}
              onChange={(e) =>
                setTs({ bg: { ...bg, padding: parseInt(e.target.value) || 0 } })
              }
              className="h-7 text-xs"
            />
          </div>
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setTs({
                  bg: { ...bg, borderRadius: parseInt(e.target.value) || 0 },
                })
              }
              className="h-7 text-xs"
            />
        ...
        <div className="grid grid-cols-2 gap-1.5 items-end">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setTs({ bg: { ...bg, borderColor: e.target.value } })
              }
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setTs({
                  bg: { ...bg, borderWidth: parseInt(e.target.value) || 0 },
                })
              }
              className="h-7 text-xs"
            />
